...
contract Shoping {
    ...
    constructor() {
        user[0x5B38Da6a701c568545dCfcB03FcB875f56beddC4] = User(
            ...
            "petrov",
            keccak256(abi.encodePacked("petr")),
            1,
            false,
            false
        );
        ...
        user[0xdD870fA1b7C4700F2BD7f44238821C26f7392148] = User(
            ...
            "Salesman",
            "salesman",
            keccak256(abi.encodePacked("salesman")),
            2,
            false,
            true
        );
    ...
    modifier CheckRequestToSalesman() {
        ...
        for (uint256 i = 0; i < requestToSalesman.length; i++) {
            if (requestToSalesman[i].addressShoper == msg.sender) {
                require(
                    requestToSalesman[i].finished == true,
                    "You are in request to salesman"
                );
            }
        }
        _;
    ...
    modifier CheckRequestToAdmin() {
        ...
        for (uint256 i = 0; i < requestToAdmin.length; i++) {
            if (requestToAdmin[i].addressUser == msg.sender) {
                require(
                    requestToAdmin[i].finished == true,
                    "You are in request to admin"
                );
            }
        }
        _;
    ...
    modifier CheckRequestToShoper() {
        ...
        for (uint256 i = 0; i < requestToShoper.length; i++) {
            if (requestToShoper[i].addressSalesman == msg.sender) {
                require(
                    requestToShoper[i].finished == true,
                    "You are in request to shoper"
                );
            }
        }
        _;
    ...
    function regUser(
        ...
        CheckLogin(login)
        CheckTheCorrectPassword(password)
    {
    ...
    function RequestToAdminFunc() public IsUser CheckRequestToAdmin {
        ...
        require(user[msg.sender].salesman != true, "You are a salesman");
        requestToAdmin.push(
            RequestToAdmin(requestToAdmin.length, msg.sender, false)
        );
    ...
    function AccRequestAdmin(uint256 requestId) public IsAdmin {
        ...
        require(
            requestToAdmin[requestId].finished == false,
    ...
    function AccRequestBank(uint256 shopId) public payable {
        ...
        require(
            shop[shopId].bankMoney == false,
            "The shop is already have money"
        );
        require(
            BankRequestShop[shopId] == true,
            "The shop is not send a request to have money"
        );
    ...
    function getShopComplainAndSuggestion(address addressShop)
        external
        view
        returns (ComplaintsAndSuggestions[] memory)
    {
        return bookOfComplaintsAndSuggestions[addressShop];
    }

    function getBeAdminRequests()
        external
        view
        returns (RequestToAdmin[] memory)
    {
        return requestToAdmin;
    }

    function getBeSalesmanRequests()
        external
        view
        returns (RequestToSalesman[] memory)
    {
        return requestToSalesman;
    }

    function getBeBuyerRequests()
        external
        view
